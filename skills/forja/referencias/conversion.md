...
2. **¿Sin capa de texto?** Si `chars/5pp` es muy bajo (≈ < 500, o sea < ~100 por
   ...
   persa, latín), usa la skill `referencias/ocr.md` (modelos best + preprocesado + RapidOCR).
...
Para el checklist completo usa la skill **`referencias/qa-conversion.md`**. **No pases a
NotebookLM ni a `referencias/traducir.md` hasta que la verificación de completitud dé 0
...
`referencias/traducir.md`. El `.md` también va directo a NotebookLM ([[notebooklm]]).
